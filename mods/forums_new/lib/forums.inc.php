...
if (!defined('AT_FORUMS_NEW__DIR')) { print "You forgot lib/module.inc.php"; exit; }
...
function print_entry($row) {
    ...
    $reply_link = '<a href="' . AT_FORUMS_NEW__DIR . '/forum/view.php?fid='.$row['forum_id'].SEP.'pid=';
    if ($row['parent_id'] == 0) {
            ...
            <div>
                <div class="forum-post-ctrl">
                    <?php if (authenticate(AT_PRIV_FORUMS, AT_PRIV_RETURN)): ?>
                        <?php echo $reply_link; ?> | <a href="<?php echo AT_FORUMS_NEW__DIR; ?>/edit_post.php?fid=<?php echo $row['forum_id'].SEP.'pid='.$row['post_id']; ?>"><?php echo _AT('edit'); ?></a> | <a href="<?php echo AT_FORUMS_NEW__DIR; ?>/forum/delete_thread.php?fid=<?php echo $row['forum_id'].SEP.'pid='.$row['post_id'].SEP.'ppid='.$row['parent_id'].SEP; ?>nest=1"><?php echo _AT('delete'); ?></a>
                    <?php elseif (($row['member_id'] == $_SESSION['member_id']) && (($row['udate'] + $forum_info['mins_to_edit'] * 60) > time())): ?>
                        <?php echo $reply_link; ?> | <a href="<?php echo AT_FORUMS_NEW__DIR; ?>/edit_post.php?fid=<?php echo $row['forum_id'].SEP.'pid='.$row['post_id']; ?>"><?php echo _AT('edit'); ?></a> <span>(<?php echo _AT('edit_for_minutes', round((($row['udate'] + $forum_info['mins_to_edit'] * 60) - time())/60)); ?>)</span>
